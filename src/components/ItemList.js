...
const ItemList = (props) => {
  ...
  const { id,name, price, description, defaultPrice, imageId } = props?.card?.info;
...
const handleRemoveItems = (item) => {
  ...
  const cartItemIncreaseDecrease=(props)=>{
    return (
      <>
      {count<1 ?<button className=" p-1" onClick={()=>handleAddItems(props)}>ADD</button> :
      (<><button className="mr-2 ml-2" onClick={()=>handleRemoveItems(props)}>-</button>
      <span>{count}</span>
      <button className="ml-2 mr-2 " onClick={()=>handleAddItems(props)}>+</button></>)}
      </>
    )
  }
  ...
  return (
    <div className="p-2 m-2  border-gray-200 border-b-2 text-left flex ">
      ...
      <div className="w-3/12 p-4 relative">
        ...
        <button  className="p-0.5   rounded-lg bg-black shadow-lg text-white">{cartItemIncreaseDecrease({...props})}</button>
        </div>  
